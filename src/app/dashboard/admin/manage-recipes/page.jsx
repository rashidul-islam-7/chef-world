...
const getCleanId = (id) => {
  if (!id) return "";
  if (typeof id === "object") {
    return String(id.$oid || id.toString());
  }
  return String(id);
};
...
const ManageRecipesPage = async () => {
  ...
  // Featured IDs গুলোকে পরিষ্কার করে Safe String এ রূপান্তর
  const formattedFeaturedIds =
    featuredRecipes?.map((item) =>
      getCleanId(item.recipeId || item._id || item),
    ) || [];
      ...
      {!data?.length ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {data.map((recipe, index) => {
              // Safe Clean ID
              const currentRecipeId = getCleanId(recipe._id);

              const isFeatured = formattedFeaturedIds.includes(currentRecipeId);

              return (
                <RecipeCard
                  key={currentRecipeId || index}
                  recipe={recipe}
                  index={index}
                  isFeatured={isFeatured}
                />
